...
import EachInteractiveXCoordinate from "./wrapper/EachInteractiveXCoordinate";
...
class InteractiveYCoordinate extends Component {
	constructor(props) {
		...
		this.getSelectionState = isHoverForInteractiveType("xCoordinateList")
			.bind(this);
	...
	handleDragComplete(moreProps) {
		...
		if (isDefined(override)) {
			const { xCoordinateList } = this.props;
			const newAlertList = xCoordinateList
				.map((each, idx) => {
	...
	handleDelete(index, moreProps) {
		const { onDelete, xCoordinateList } = this.props;
		onDelete(xCoordinateList[index], moreProps);
	}
	render() {
		const { xCoordinateList } = this.props;
		const { override } = this.state;
		return (
			<g>
				{xCoordinateList.map((each, idx) => {
					const props = each;
					return (
						<EachInteractiveXCoordinate key={each.id}
...
InteractiveYCoordinate.propTypes = {
	...
	xCoordinateList: PropTypes.array.isRequired,
